...
export const DataContextProvider = ({ children }) => {
  ...
  const handleProductUpload = async (e) => {
    ...
    if (showEditText) {
      ...
      try {
        const response = await axios.post(`http://localhost:5000/editProduct/${dashboardProductDetails._id}`, formData, {
          headers: {
  ...
  const handleCategoryUpload = async (e) => {
    ...
    if (showEditText) {
      ...
      try {
        const response = await axios.post(`http://localhost:5000/editCategory/${dashboardProductDetails._id}`, formData, {
          headers: {
            "Content-Type": "multipart/form-data"
          }
        });
      ...
      try {
        const response = await axios.post("http://localhost:5000/uploadCategory", formData, {
          headers: {
            "Content-Type": "multipart/form-data"
          }
        });
  ...
  useEffect(()=>{
    const fetchRazorpayKey=async()=>{
      try {
        const response = await axios.get("http://localhost:5000/razorpayKey");
  ...
  const dashboarProductDelete = async (id) => {
    try {
      const response = await axios.get(`http://localhost:5000/productDelete/${id}`);
      setIsAddProduct(response.data)
    } catch (err) {
      console.log("Error fetching products:", err);
  ...
  const dashboarCategoryDelete = async (id) => {
    try {
      const response = await axios.get(`http://localhost:5000/categoryDelete/${id}`);
  ...
  const filterData = async (filterData) => {
    try {
      const response = await axios.get('http://localhost:5000/filterProduct', { params: filterData })
      setAllProductData(response.data)
  ...
  const filterDataBySearch = async (filterData) => {
    try {
      const response = await axios.get('http://localhost:5000/filterProductBySearch', { params:{name: filterData} })
      setAllProductData(response.data)
      setNotFound(response.data.message)
    }
    catch (err) {
    ...
    const filterPriceData = async (filterPrice) => {
        ...
        try{
          const response= await axios.get('http://localhost:5000/filterByPrice',{params :filterPrice})
          setAllProductData(response.data)
    ...
    const handleUserSignUp = async (userSignUpData) => {
      ...
      try {
        const response = await axios.post('http://localhost:5000/signUp', userSignUpData, {
          headers: {
    ...
    const handleUserSignIn = async (userSignInData) => {
     ...
     const handleAddToCartData = async (userId, productId,quantity) => {
      ...
      try {
        const response = await axios.post('http://localhost:5000/addToCart', { userId, productId, quantity }, {
          headers: {
            'Content-Type': 'application/json'
          }
        });
      ...
      const handleUpdateCartData = async (userId, productId,count) => {
        ...
        try {
          const response = await axios.post('http://localhost:5000/updateCart', { userId, productId, count }, {
            headers: {
              'Content-Type': 'application/json'
            }
          });
      ...
      const handleDeleteCart= async(userId,productId)=>{
        try {
          const response = await axios.get('http://localhost:5000/deleteCartItem', {
            params: {
              userId,
              productId
            }
      ...
      useEffect(()=>{
        ...
        const cartFetch=async()=>{
          ...
          if (!user._id) {
            ...
            try {
              const response = await axios.get(`http://localhost:5000/cartData/${user._id}`);
      ...
      useEffect(()=>{
        ...
        const cartFetch=async()=>{
          // console.log(user._id)
            ...
            try {
              const response = await axios.get(`http://localhost:5000/allCartData`);
